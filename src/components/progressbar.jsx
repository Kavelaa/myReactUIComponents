  ...
  componentDidMount() {
    if (this.props.switch) {
      if (this.props.mode === 'timer') {
        ...
        this.paintTimer = setInterval(this.counter, duration * 5, ctx, this.count)
      }
      else if (this.props.mode === 'progress') {
        if (isNaN(this.props.progress) || this.props.progress < 0 || this.props.progress > 200) {
          throw new Error('Invalid value, scope should be 0 to 200!')
        }
        let [w, h, c, ctx] = [this.props.width, this.props.height, this.props.progress, this.ctx]
